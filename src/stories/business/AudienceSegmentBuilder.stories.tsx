...
const segments: TransferItem[] = [
  ...
  {
    key: "at-risk",
    label: "At-risk repeat buyers",
    description: "No order in 45+ days",
  },
  {
    key: "new",
  ...
  {
    key: "retail",
    label: "Retail event attendees",
    description: "Scanned at offline events",
  },
  {
    key: "coupon",
    label: "Coupon redeemers",
    description: "Used promo in last campaign",
  },
];

function InteractiveSegmentBuilder(args: AudienceSegmentBuilderProps) {
  const [selectedKeys, setSelectedKeys] = useState(
    args.selectedSegmentKeys ?? ["vip"],
  );
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_18rem]">
      <AudienceSegmentBuilder
        ...
        onFiltersChange={(result) =>
          setFilterSummary(
            `${result.logic} with ${result.filters.length} rule filter(s)`,
          )
        }
      />
      <div className="bg-muted/30 rounded-xl border p-4 text-sm">
        ...
        <p className="text-muted-foreground mt-2">{filterSummary}</p>
      </div>
    </div>
  );
}
