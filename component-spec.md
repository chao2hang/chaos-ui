...
```
├── src/                     # Storybook component showcase
│   ├── intro/               # Introduction & getting started docs
...
│   ├── feature-flag.ts      # Feature flag helpers
...
`components/ui/` contains two kinds of components:

**shadcn base components** — generated by shadcn CLI. **DO NOT MODIFY** unless absolutely necessary. If customization is needed, create a wrapper in `components/business/`. Examples: `button.tsx`, `dialog.tsx`, `card.tsx`, `select.tsx`, `table.tsx`.

**Composite UI components** — custom components built by composing shadcn primitives and other libraries. These **can be modified**. Examples: `stepper.tsx`, `file-upload.tsx`, `virtual-list.tsx`, `color-picker.tsx`, `tags-input.tsx`, `tree-select.tsx`.
...
- ❌ Adding arbitrary custom CSS utilities inline (use Tailwind classes)
