...
describe("OrgChart", () => {
  ...
  it("passes nodes and edges to ReactFlow", () => {
    ...
    expect(getByTestId("node-1").textContent).toBe("Alice");
    expect(getByTestId("node-2").textContent).toBe("Bob");
  });

  it("applies custom className", () => {
    const { container } = render(
      <OrgChart nodes={nodes} edges={edges} className="custom-org" />,
    );
  ...
  it("calls onNodeClick when a node is clicked", () => {
    ...
    const { getByTestId } = render(
      <OrgChart nodes={nodes} edges={edges} onNodeClick={handleClick} />,
    );
  ...
  it("module is importable", async () => {
    const mod = await import("@/components/business/org-chart");
    expect(mod.OrgChart).toBeDefined();
  });
});
