...
const badgeVariant: Record<DiffChangeType, "default" | "secondary" | "destructive" | "outline"> = {
  added: "default",
  removed: "destructive",
  changed: "secondary",
  unchanged: "outline",
...
export function DiffViewer({
  ...
  return (
    <div data-slot="diff-viewer" className={cn("overflow-hidden rounded-lg border", className)} {...props}>
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(type === "removed" && "text-destructive line-through")}>{item.before ?? "-"}</span>
              <span className={cn(type === "added" && "text-success", type === "changed" && "font-medium")}>{item.after ?? "-"}</span>
