...
function ApprovalFlow({ nodes, edges, className }: ApprovalFlowProps) {
  const order = React.useMemo(() => {
    ...
    while (cur && !seen.has(cur)) { seen.add(cur); const n = nodes.find((x) => x.id === cur); if (n) out.push(n); cur = next.get(cur); }
    if (out.length === 0) return nodes;
    return out;
  }, [nodes, edges]);
  return (
    <div data-slot="approval-flow" className={cn("flex flex-wrap items-center gap-2", className)}>
      {order.map((n, i) => (
        <React.Fragment key={n.id}>
          <div className="flex items-center gap-2 rounded-lg border bg-card px-3 py-2 text-sm">
            <span className={cn("flex size-5 items-center justify-center rounded-full text-xs", STATUS_STYLE[n.status ?? "pending"] ?? "bg-muted")}>
              {n.status === "approved" ? <CheckIcon className="size-3" /> : n.status === "rejected" ? <XIcon className="size-3" /> : <ClockIcon className="size-3" />}
            </span>
            <span className="font-medium">{n.name}</span>
          </div>
          {i < order.length - 1 && <ChevronRightIcon className="size-4 text-muted-foreground" />}
        </React.Fragment>
      ))}
    </div>
  );
