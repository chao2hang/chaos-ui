...
vi.mock("@/components/ui/icons", () => ({
  ...
  ArrowLeftIcon: (p: Record<string, unknown>) => (
    <svg data-testid="arrow-left" {...p} />
  ),
...
const sourceColumns: SourceColumn[] = [
  ...
  },
  {
    ...
    index: 1,
    detectedType: "number",
    samples: ["9.99", "19.99", "29.99"],
  },
  {
    name: "created_date",
    index: 2,
    detectedType: "date",
    samples: ["2026-01-01", "2026-02-01"],
  },
  {
    name: "sku_code",
    index: 3,
...
describe("ImportMappingWizard", () => {
  it("renders with data-slot", () => {
    ...
    expect(
      container.querySelector('[data-slot="import-mapping-wizard"]'),
    ).toBeTruthy();
  });
  ...
  it("auto-match button re-runs matching", () => {
    ...
    expect(
      updated.filter((m: { source: string }) => m.source).length,
    ).toBeGreaterThan(0);
  });
