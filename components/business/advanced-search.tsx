...
export interface AdvancedSearchProps extends Omit<React.ComponentProps<"form">, "onSubmit"> {
...
export function AdvancedSearch({
  ...
  const [values, setValues] = React.useState<AdvancedSearchValues>(defaultValues)
    ...
    >
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="flex-1 space-y-1.5">
          <Label htmlFor="advanced-search-query">Search</Label>
          <div className="relative">
            <SearchIcon className="absolute left-2 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input
              id="advanced-search-query"
              className="pl-8"
              placeholder="Search campaigns, owners, or IDs"
              value={values.query ?? ""}
      ...
      <div className="flex justify-end gap-2">
        {onSaveSearch && (
          <Button type="button" variant="outline" onClick={() => onSaveSearch(values)}>
            <StarIcon />
            Save filter
          </Button>
        )}
