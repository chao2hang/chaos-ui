...
function ResizablePanel({
  ...
  return (
    <ResizablePanelHandleProvider
      value={{ size, setSize, minSize, maxSize, collapsed, collapsedSize, collapsible, handleCollapse, handleExpand, direction: ctx.direction }}
    >
      <div
...
function ResizablePanelHandleProvider({
  ...
  return <ResizableHandleContext.Provider value={value}>{children}</ResizableHandleContext.Provider>
}
  ...
  const onPointerDown = React.useCallback(
    (e: React.PointerEvent) => {
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        const next = Math.min(Math.max(startSize.current + delta * 100, minSize), maxSize)
        setSize(next)
      }
