...
export function SignInForm({ onSubmit, ssoProviders, signupHref, forgotHref, className, ...props }: SignInFormProps) {
    ...
    <form onSubmit={handleSubmit} className={cn("space-y-4", className)} {...props}>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          {forgotHref && (
            <Link href={forgotHref} className="text-xs text-primary hover:underline">忘记密码？</Link>
          )}
        </div>
      ...
      {ssoProviders && ssoProviders.length > 0 && (
        <>
          ...
          <div className="grid gap-2">
            {ssoProviders.map((p) => (
              <Button key={p.id} type="button" variant="outline" className="w-full">
                {p.icon} {p.label}
              </Button>
            ))}
          </div>
      ...
      {signupHref && (
        <p className="text-center text-sm text-muted-foreground">
          还没有账号？ <Link href={signupHref} className="text-primary hover:underline">注册</Link>
        </p>
      )}
    </form>
  )
...
export function SignUpForm({ onSubmit, signinHref, className, ...props }: SignUpFormProps) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const fd = new FormData(e.currentTarget)
    onSubmit?.({
  ...
  return (
    <form onSubmit={handleSubmit} className={cn("space-y-4", className)} {...props}>
      ...
      {signinHref && (
        <p className="text-center text-sm text-muted-foreground">
          已有账号？ <Link href={signinHref} className="text-primary hover:underline">登录</Link>
        </p>
      )}
    </form>
  )
...
export function ForgotPasswordForm({ onSubmit, className, ...props }: ForgotPasswordFormProps) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
