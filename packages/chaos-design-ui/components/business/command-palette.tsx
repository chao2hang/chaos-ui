...
export function CommandPalette({
  ...
  React.useEffect(() => {
    ...
    const isMod = parts.includes("mod") || parts.includes("cmd") || parts.includes("ctrl")
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 border-b px-3">
            ...
            />
            {showShortcut && <KbdGroup className="ml-auto">{parseShortcut(shortcut)}</KbdGroup>}
          </div>
...
function parseShortcut(shortcut: string): React.ReactNode {
  return shortcut.split("+").map((k, i) => (
    <Kbd key={i} size="sm">
      {k.trim() === "mod" || k.trim() === "cmd" ? "⌘" : k.trim() === "ctrl" ? "Ctrl" : k.trim() === "shift" ? "⇧" : k.trim() === "alt" ? "⌥" : k.trim()}
    </Kbd>
  ))
}
