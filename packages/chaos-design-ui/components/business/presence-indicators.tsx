...
interface PresenceIndicatorsProps extends React.ComponentProps<"div"> {
  users: PresenceUser[]
  max?: number
  className?: string
}

const STATUS_COLORS: Record<PresenceStatus, "success" | "warning" | "destructive" | "default"> = {
  online: "success",
  away: "warning",
  busy: "destructive",
  offline: "default",
...
export function PresenceIndicators({ users, max = 5, className, ...props }: PresenceIndicatorsProps) {
  const visible = users.slice(0, max)
  const overflow = users.length - max
  return (
    <div data-slot="presence-indicators" className={cn("flex -space-x-2", className)} {...props}>
      {visible.map((u) => (
        <div key={u.id} className="relative">
          <Avatar className="size-8 border-2 border-background" title={`${u.name} · ${u.status}`}>
