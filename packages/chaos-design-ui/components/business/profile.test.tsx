...
describe("ProfileHeader", () => {
  ...
  it("renders department when provided", () => {
    render(
      <ProfileHeader
        user={{ name: "Alice", role: "工程师", department: "平台组" }}
      />,
    )
    expect(screen.getByText("工程师")).toBeInTheDocument()
  ...
  it("renders email as mailto link", () => {
    render(<ProfileHeader user={{ name: "Alice", email: "a@x.com" }} />)
    const link = screen.getByRole("link", { name: "a@x.com" })
    expect(link).toHaveAttribute("href", "mailto:a@x.com")
  })
