...
export function formatRelativeTime(
  ...
  for (const [seconds, u] of ranges) {
    if (abs < seconds) {
      unit = u
      divisor = unit === "second" ? 1 : unit === "minute" ? 60 : unit === "hour" ? 3600 : unit === "day" ? 86400 : unit === "month" ? 2592000 : 31536000
      break
    }
  }
...
export function formatPercent(value: number, fractionDigits = 1, locale = "zh-CN"): string {
...
export function formatDuration(seconds: number, locale = "zh-CN"): string {
  ...
  if (h > 0) return new Intl.NumberFormat(locale, { style: "unit", unit: "hour", unitDisplay: "narrow" }).format(h)
  if (m > 0) return new Intl.NumberFormat(locale, { style: "unit", unit: "minute", unitDisplay: "narrow" }).format(m)
