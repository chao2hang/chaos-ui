...
function emit(
  metric: Omit<
    WebVitalMetric,
    "id" | "rating" | "entries" | "navigationType" | "delta"
  > & { entries?: PerformanceEntry[]; delta?: number },
) {
...
export function startWebVitals() {
  ...
  const tryObserve = (
    type: string,
    name: WebVitalName,
    getValue: (e: PerformanceEntry) => number,
  ) => {
    try {
  ...
  const cleanups = [
    ...
    tryObserve(
      "layout-shift",
      "CLS",
      (e) => (e as unknown as { value: number }).value,
    ),
    tryObserve(
      "first-input",
      "FID",
      (e) =>
        (e as unknown as { processingStart: number; startTime: number })
          .processingStart - e.startTime,
    ),
    ...
    tryObserve(
      ...
      "TTFB",
      (e) =>
        (e as unknown as { responseStart: number; requestStart: number })
          .responseStart -
        (e as unknown as { requestStart: number }).requestStart,
    ),
  ];
