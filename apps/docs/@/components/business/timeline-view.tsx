...
function TimelineView({ events = [], className }: TimelineViewProps) {
  return (
    ...
    >
      {events.length === 0 ? (
        <li className="text-muted-foreground flex items-center gap-3 py-6 text-sm">
        ...
        events.map((event, idx) => {
          ...
          return (
            <li key={event.id} className="relative flex gap-3 pb-6 last:pb-0">
              {!isLast && (
                <span
                  className="bg-border absolute top-4 bottom-0 left-[7px] w-px"
                  aria-hidden="true"
                />
              )}
              <span
                className="border-primary bg-background relative z-10 mt-1 flex size-3.5 shrink-0 items-center justify-center rounded-full border-2"
                aria-hidden="true"
              />
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline gap-2">
                  <time
                    dateTime={event.date}
                    className="text-primary shrink-0 text-xs font-medium tabular-nums"
                  >
