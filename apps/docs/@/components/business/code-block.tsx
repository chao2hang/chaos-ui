...
function tokenize(code: string, language: string): React.ReactNode[] {
  ...
  for (let i = 0; i < regs.length; i++) {
    ...
    while ((m = re.exec(code)) !== null) {
      tokens.push({ start: m.index, end: m.index + m[0].length, type: ["comment", "keyword", "string", "number"][i] })
    }
  }
    ...
    >
      {filename && (
        <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 px-3 py-1.5 text-xs text-zinc-400">
          ...
          {language && <span className="text-[0.65rem] uppercase">{language}</span>}
        </div>
      )}
      {showCopy && (
        <Button
          ...
          aria-label="复制代码"
        >
          {copied ? <CheckIcon className="text-zinc-100" /> : <CopyIcon className="text-zinc-100" />}
        </Button>
      )}
      <pre
            ...
            <table className="w-full border-collapse">
              <tbody>
                {lines.map((_, i) => (
                  <tr
                    key={i}
                    className={cn(highlightLines.includes(i + 1) && "bg-zinc-800/60")}
                  >
                    <td className="select-none pr-4 text-right align-top text-zinc-600">
                      {i + 1}
                    </td>
                    <td className="align-top whitespace-pre">{lines[i] || " "}</td>
                  </tr>
                ))}
              </tbody>
            </table>
