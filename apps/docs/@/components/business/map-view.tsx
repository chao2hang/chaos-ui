...
const amapWindow = (
  typeof window === "undefined" ? ({} as AMapWindow) : window
) as Window & AMapWindow & typeof globalThis;
...
function loadAmapScript(apiKey?: string, version = "2.0"): Promise<AMapStatic> {
  ...
  amapScriptPromise = new Promise<AMapStatic>((resolve, reject) => {
    ...
    if (!key) {
      reject(
        new Error(
          "MapView 缺少 apiKey:请通过 props.apiKey 传入高德地图 JS API key",
        ),
      );
      amapScriptPromise = null;
      return;
    ...
    >
      ...
      {!isReady && (
        <div className="bg-muted absolute inset-0 flex items-center justify-center">
          <div className="text-muted-foreground text-sm">地图加载中...</div>
        </div>
      )}
    </div>
  );
