 ...
 * @category Business
 * @since 1.0.0-beta.0
 * @description 部门选择器（支持树形）：combobox 触发 + 可筛选的树形弹层。
 ...
 *   value="D2"
 *   onChange={setId}
 *   options={[{ value: "D1", label: "总部" }, { value: "D2", label: "销售部", parent: "D1" }]}
 * />
 * ```
 */
interface DepartmentPickerOption {
  value: string;
  label: string;
  /** Parent value (forms a tree). / 父节点 value（构成树） */
  parent?: string;
  disabled?: boolean;
}
...
function DepartmentPicker({
  ...
  const rows = React.useMemo(() => {
    ...
    // When searching, show matches (plus ancestors for context), flat with depth 0.
        ...
        <span className="flex items-center gap-1">
          {clearable && selected && (
            <span
              role="button"
              tabIndex={0}
              aria-label={t("departmentPicker.clear", { defaultValue: "清除" })}
              onClick={(e) => {
                e.stopPropagation();
                onChange?.(undefined);
              }}
...
export type { DepartmentPickerProps, DepartmentPickerOption };
