    ...
    <nav data-slot="anchor" className={cn("space-y-1 text-sm", className)}>
      {sections.map((s) => (
        <a
          ...
          className={cn(
            "block rounded-sm py-1 text-muted-foreground transition-colors hover:text-foreground",
            active === s.id && "font-medium text-primary",
            s.level && s.level > 1 && `pl-${(s.level - 1) * 3}`
          )}
          style={s.level && s.level > 1 ? { paddingLeft: `${(s.level - 1) * 12}px` } : undefined}
        >
          {s.label}
        </a>
      ))}
