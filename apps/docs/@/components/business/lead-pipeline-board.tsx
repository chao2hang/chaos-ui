...
const stageColorMap: Record<string, { bar: string; badge: string; header: string }> = {
  blue: { bar: "bg-blue-500", badge: "bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300", header: "text-blue-600" },
  indigo: { bar: "bg-indigo-500", badge: "bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300", header: "text-indigo-600" },
  violet: { bar: "bg-violet-500", badge: "bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300", header: "text-violet-600" },
  amber: { bar: "bg-amber-500", badge: "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300", header: "text-amber-600" },
  emerald: { bar: "bg-emerald-500", badge: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300", header: "text-emerald-600" },
  rose: { bar: "bg-rose-500", badge: "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300", header: "text-rose-600" },
};
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        ...
        <div className="flex items-center gap-4 text-sm">
          ...
          <div className="text-muted-foreground">
            Total: <strong className="text-foreground tabular-nums">{formatMoney(totalValue, currencySymbol)}</strong>
          </div>
          <div className="text-muted-foreground">
            Weighted: <strong className="text-foreground tabular-nums">{formatMoney(weightedTotal, currencySymbol)}</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${stages.length}, minmax(200px, 1fr))` }}>
        {stages.map((stage) => {
          ...
          const colors = stageColorMap[stage.color ?? "blue"] ?? stageColorMap.blue!;
          ...
          return (
            ...
            >
              {/* Column header */}
              <div className={cn("rounded-t-lg border-b border-border px-3 py-2")}>
                <div className="flex items-center justify-between">
                  <span className={cn("text-sm font-semibold", colors.header)}>{stage.label}</span>
                  <span className={cn("rounded-full px-1.5 py-0.5 text-xs font-medium", colors.badge)}>
                    {stage.probability}%
                  </span>
                </div>
                <div className="mt-1 flex items-center justify-between text-xs text-muted-foreground">
                  <span>{stageDeals.length} deals</span>
                  <span className="tabular-nums">{formatMoney(stageValue, currencySymbol)}</span>
                </div>
                <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-muted">
                  <div className={cn("h-full transition-all", colors.bar)} style={{ width: `${stage.probability}%` }} />
                </div>
              </div>

              {/* Deal cards */}
              <div className="flex-1 space-y-2 overflow-y-auto p-2" style={{ maxHeight: "400px" }}>
                {stageDeals.length === 0 ? (
                  <div className="py-4 text-center text-xs text-muted-foreground">No deals</div>
                ) : (
                  stageDeals.map((deal) => (
                    <div
                      key={deal.id}
                      ...
                      className={cn(
                        "cursor-pointer rounded-md border border-border border-l-4 bg-card p-2.5 shadow-sm transition-shadow hover:shadow-md",
                        priorityColors[deal.priority ?? "low"],
                        draggedDeal === deal.id && "opacity-50",
                      )}
                    >
                      <div className="text-sm font-medium text-foreground">{deal.title}</div>
                      <div className="mt-0.5 text-xs text-muted-foreground">{deal.customer}</div>
                      <div className="mt-1.5 flex items-center justify-between">
                        <span className="text-sm font-bold tabular-nums text-foreground">
                          {formatMoney(deal.value, deal.currency ? "" : currencySymbol)}
                        </span>
                        {deal.owner && (
