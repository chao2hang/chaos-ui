    ...
    >
      <div className="overflow-auto" style={{ height }}>
        <table className="w-full text-xs">
          <thead>
            <tr>
              <th className="bg-background sticky left-0 p-1"></th>
              {matrix.xs.map((x) => (
                <th key={x} className="text-muted-foreground p-1 text-xs">
                  {x}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {matrix.ys.map((y) => (
              <tr key={y}>
                <th className="bg-background text-muted-foreground sticky left-0 p-1 text-left text-xs">
                  {y}
                </th>
                {matrix.xs.map((x) => {
