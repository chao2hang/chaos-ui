...
import { CookieIcon, XIcon } from "@chaos_team/chaos-ui/ui-icons";
...
export function CookieBanner({
  ...
  const resolvedTitle =
    title === "Cookie 使用提示" ? t("cookieBanner.title") : title;
  const resolvedDescription =
    description ===
    "我们使用 Cookie 提升您的体验。点击「接受」以启用全部功能，或在设置中自定义。"
      ? t("cookieBanner.description")
      : description;
  const resolvedAcceptText =
    acceptText === "接受全部" ? t("cookieBanner.accept") : acceptText;
  const resolvedRejectText =
    rejectText === "仅必要" ? t("cookieBanner.reject") : rejectText;
  const resolvedPolicyText =
    policyText === "隐私政策" ? t("cookieBanner.policy") : policyText;
  ...
  const positionClass: Record<
    NonNullable<CookieBannerProps["position"]>,
    string
  > = {
