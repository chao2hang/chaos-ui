...
import { CalendarIcon, CarIcon, ClockIcon } from "@/components/ui/icons";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border pb-3">
        ...
        <h3 className="text-lg font-semibold text-foreground">Vehicle Booking</h3>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {vehicles.map((v) => {
            ...
            const statusInfo = (statusConfig[v.status] ?? statusConfig.available)!;
            return (
              <button
                key={v.id}
                type="button"
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-muted-foreground">{typeLabels[v.type]}</span>
                    <span className="text-muted-foreground">·</span>
                    <span className="text-muted-foreground">{v.seats} seats</span>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Applicant</label>
          <Input value={applicant} onChange={(e) => setApplicant(e.target.value)} disabled={readOnly} aria-label="Applicant name" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Destination</label>
          <Input value={destination} onChange={(e) => setDestination(e.target.value)} disabled={readOnly} aria-label="Destination" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Start Time</label>
          <div className="relative">
            <ClockIcon className="absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input type="datetime-local" className="pl-8" value={startTime} onChange={(e) => setStartTime(e.target.value)} disabled={readOnly} aria-label="Start time" />
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <ClockIcon className="absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
            <Input type="datetime-local" className="pl-8" value={endTime} onChange={(e) => setEndTime(e.target.value)} disabled={readOnly} aria-label="End time" />
          </div>
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Passengers</label>
          <Input type="number" min={1} max={selectedVehicleObj?.seats ?? 50} value={passengers} onChange={(e) => setPassengers(parseInt(e.target.value) || 1)} disabled={readOnly} aria-label="Passengers" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Driver (optional)</label>
          <Input value={driver} onChange={(e) => setDriver(e.target.value)} disabled={readOnly} aria-label="Driver" />
        </div>
      </div>
      ...
      {/* Conflict warning */}
      {endTimeBeforeStart && (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive" data-slot="time-error">
          End time must be after start time
        </div>
      )}
      {hasConflict && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2" data-slot="conflict-warning">
          <div className="text-xs font-medium text-amber-700 dark:text-amber-400">
            ⚠ {conflicts.length} booking conflict{conflicts.length > 1 ? "s" : ""} detected:
          </div>
          {conflicts.map((c) => (
            <div key={c.id} className="mt-1 text-xs text-amber-600 dark:text-amber-500">
      ...
      {selectedVehicle && (
        <div data-slot="vehicle-bookings">
          ...
          <div className="space-y-1.5">
            {bookings
              .filter((b) => b.vehicleId === selectedVehicle && (b.status === "approved" || b.status === "pending"))
              .map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 text-sm text-foreground">{b.applicant}</span>
            ...
            {bookings.filter((b) => b.vehicleId === selectedVehicle && (b.status === "approved" || b.status === "pending")).length === 0 && (
              <div className="py-2 text-center text-xs text-muted-foreground">No existing bookings</div>
            )}
          </div>
        </div>
      )}
      ...
      {!readOnly && (
        <div className="flex justify-end gap-2 border-t border-border pt-3">
          ...
          <Button onClick={handleSubmit} disabled={!canSubmit}>Submit Booking</Button>
        </div>
      )}
    </div>
  );
