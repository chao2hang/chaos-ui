...
export function VideoPlayer({
  ...
  const seek = React.useCallback(
    (seconds: number) => {
      ...
      video.currentTime = Math.max(
        0,
        Math.min(video.currentTime + seconds, maxTime),
      );
    },
    [duration],
  );
  ...
  return (
    <div
      ...
      className={cn(
        "group relative overflow-hidden rounded-lg bg-black outline-none",
        className,
      )}
    ...
    >
      ...
      <div
        data-controls
        className={cn(
          "absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-4 pt-12 pb-4 transition-opacity duration-300",
      ...
      >
        ...
        >
          <div className="absolute inset-y-0 right-0 left-0 flex items-center">
            <div className="relative h-1 w-full rounded-full bg-white/30 transition-[height] group-hover/progress:h-1.5">
              <div
                className="bg-primary absolute inset-y-0 left-0 rounded-full"
                style={{ width: `${progressPercent}%` }}
              />
              <div
                className="bg-primary absolute top-1/2 size-3 -translate-x-1/2 -translate-y-1/2 rounded-full opacity-0 shadow transition-opacity group-hover/progress:opacity-100"
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="group/volume relative flex h-8 items-center">
              <button
                ...
                className="flex size-8 shrink-0 items-center justify-center rounded-full transition-colors hover:bg-white/20"
              ...
              <div className="h-8 w-0 overflow-hidden transition-all duration-200 group-hover/volume:w-[5.5rem]">
                <div className="flex h-full items-center px-1">
                  <input
                    ...
                    className="accent-primary h-1 w-20 cursor-pointer appearance-none rounded-full bg-white/30"
                    aria-label="Volume"
                  />
                </div>
              </div>
