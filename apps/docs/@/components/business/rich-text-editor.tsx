...
import {
  BoldIcon, ItalicIcon, ListIcon, ListOrderedIcon, QuoteIcon, CodeIcon,
  LinkIcon, ImageIcon, PaperclipIcon, UndoIcon, RedoIcon,
  Heading1Icon, Heading2Icon, StrikethroughIcon, ExternalLinkIcon, TrashIcon,
} from "lucide-react"
...
export function RichTextEditor({
  ...
  if (!editor) {
    return <div className="rounded-md border bg-muted/30 p-4 text-sm text-muted-foreground">加载编辑器...</div>
  }

  const defaultUpload = (file: File): Promise<{ url: string; name: string; size: number }> =>
    new Promise((resolve) => {
      const reader = new FileReader()
      reader.onload = (ev) => resolve({ url: String(ev.target?.result ?? ""), name: file.name, size: file.size })
...
type UploadFn = (file: File) => Promise<{ url: string; name?: string; size?: number }>

function RichTextToolbar({ editor, onUpload }: { editor: Editor; onUpload: UploadFn }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 border-b p-1">
      <ToolButton onClick={() => editor.chain().focus().toggleBold().run()} active={editor.isActive("bold")} icon={<BoldIcon />} title="加粗" />
      <ToolButton onClick={() => editor.chain().focus().toggleItalic().run()} active={editor.isActive("italic")} icon={<ItalicIcon />} title="斜体" />
      <ToolButton onClick={() => editor.chain().focus().toggleStrike().run()} active={editor.isActive("strike")} icon={<StrikethroughIcon />} title="删除线" />
      <ToolButton onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive("heading", { level: 1 })} icon={<Heading1Icon />} title="标题1" />
      <ToolButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} active={editor.isActive("heading", { level: 2 })} icon={<Heading2Icon />} title="标题2" />
      <ToolButton onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive("bulletList")} icon={<ListIcon />} title="无序列表" />
      <ToolButton onClick={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive("orderedList")} icon={<ListOrderedIcon />} title="有序列表" />
      <ToolButton onClick={() => editor.chain().focus().toggleBlockquote().run()} active={editor.isActive("blockquote")} icon={<QuoteIcon />} title="引用" />
      <ToolButton onClick={() => editor.chain().focus().toggleCodeBlock().run()} active={editor.isActive("codeBlock")} icon={<CodeIcon />} title="代码块" />
            ...
            onApply={(url, openInNew) => {
              if (url) {
                editor
                  ...
                  .setLink({ href: url, target: openInNew ? "_blank" : null, rel: openInNew ? "noopener noreferrer" : null })
                  .run()
              } else {
      ...
      <ToolButton onClick={() => editor.chain().focus().redo().run()} icon={<RedoIcon />} title="重做" />
          ...
          editor
            ...
            .insertContent({
              ...
              marks: [
                {
                  type: "link",
                  attrs: { href: info.url, target: "_blank", rel: "noopener noreferrer" },
                },
              ],
            })
            .run()
    ...
    <div className="space-y-3" key={currentUrl ?? "new"}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="ml-auto flex gap-1">
          ...
          <Button size="xs" disabled={!isValid} onClick={() => onApply(url, openInNew)}>
            {isEdit ? "更新" : "插入"}
          </Button>
        </div>
      </div>
    ...
    <DialogContent className="sm:max-w-md">
        ...
        {tab === "upload" ? (
          <div className="space-y-3">
            ...
            >
              {preview ? (
                <img src={preview} alt={alt} className="max-h-full max-w-full rounded object-contain" />
              ) : (
                <>
                  <ImageIcon className="size-8" />
                  <span>点击或拖拽图片到此处</span>
                  <span className="text-xs text-muted-foreground">支持 PNG / JPG / GIF / WebP · 最大 10MB</span>
                </>
              )}
              <input
                ref={fileRef}
            ...
            {fileName && !preview && inserting && (
              <p className="text-xs text-muted-foreground">正在上传 {fileName}...</p>
            )}
          </div>
        ) : (
          <div className="space-y-1.5">
...
function AttachmentDialogContent({
  ...
  const [files, setFiles] = React.useState<Array<{ file: File; url?: string; error?: string }>>([])
  const [dragOver, setDragOver] = React.useState(false)
  const fileRef = React.useRef<HTMLInputElement>(null)
  ...
  const addFiles = async (incoming: FileList | File[]) => {
    ...
    for (const f of arr) {
      ...
      try {
        ...
        setFiles((prev) =>
          prev.map((entry) => (entry.file === f ? { ...entry, error: "上传失败" } : entry))
        )
      } finally {
        setInserting(false)
      }
            ...
            {files.map((entry) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-muted-foreground">{formatFileSize(entry.file.size)}</div>
                </div>
                {entry.error ? (
                  <span className="text-destructive">{entry.error}</span>
                ) : entry.url ? (
                  <a href={entry.url} target="_blank" rel="noreferrer" className="text-primary hover:underline" title="预览">
