...
import type { DashboardConfig, DashboardWidget } from "./dashboard-designer"
...
describe("DashboardDesigner", () => {
  ...
  it("add widget from catalog", async () => {
    ...
    // Click a widget type
    const menuItem = screen.getByTestId("catalog-kpi")
  ...
  it("delete widget", async () => {
    ...
    expect(newConfig.widgets.find((w) => w.id === "w1")).toBeUndefined()
  })
  ...
  it("undo reverses add", async () => {
    ...
    // Add widget
    await user.click(screen.getByTestId("add-widget-button"))
    await user.click(screen.getByTestId("catalog-kpi"))
    expect(onChange).toHaveBeenCalledTimes(1)
  ...
  it("redo re-applies undone action", async () => {
    ...
    const redoneConfig = onChange.mock.calls[2][0] as DashboardConfig
    expect(redoneConfig.widgets).toHaveLength(1)
  })
