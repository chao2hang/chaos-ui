  ...
  return (
    <div data-slot="gantt-chart" className={cn("space-y-2", className)}>
        ...
        <div className="relative h-6 border-b">
          {Array.from({ length: Math.min(totalDays, 31) }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className="text-muted-foreground absolute top-0 bottom-0 border-l text-[0.65rem]"
      ...
      <div className="space-y-1">
        {tasks.map((task) => {
          const offset =
            ((task.start.getTime() - minDate.getTime()) / 86400000) * dayWidth;
          const width =
            ((task.end.getTime() - task.start.getTime()) / 86400000) * dayWidth;
          return (
            <div
              key={task.id}
            ...
            >
              ...
              <div className="bg-muted/30 relative h-7 rounded">
                <div
      ...
      <p className="text-muted-foreground text-xs">
        ...
        {maxDate.toLocaleDateString(locale)} · {tasks.length} 个任务
      </p>
    </div>
  );
}
