...
describe("ReportBuilder", () => {
  it("renders 3-panel layout", () => {
    const { container } = render(
      <ReportBuilder definitions={testDefinitions} />,
    );
    expect(
      container.querySelector('[data-slot="report-builder"]'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('[data-slot="report-builder-palette"]'),
    ).toBeInTheDocument();
    expect(
      container.querySelector('[data-slot="report-builder-canvas"]'),
    ).toBeInTheDocument();
  });
  ...
  it("widget renders on canvas at correct position", () => {
    render(
      <ReportBuilder widgets={sampleWidgets} definitions={testDefinitions} />,
    );
    const widget = screen
      .getByTestId("delete-widget-w1")
      .closest('[data-slot="report-builder-widget"]');
