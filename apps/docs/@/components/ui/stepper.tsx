...
function Stepper({ className, activeStep = 0, orientation = "horizontal", children }: StepperProps) {
  ...
  return (
    <div
      className={cn(
        orientation === "horizontal" ? "flex items-center justify-center w-full mx-auto" : "flex flex-col",
        className
      )}
    >
...
function Step({ className, index = 0, active, completed, last, orientation, children }: StepInternalProps & React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        orientation === "horizontal" ? "flex items-center flex-1 max-w-xs min-w-0" : "flex flex-col items-start",
        className
      )}
    >
