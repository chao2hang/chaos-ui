...
const inboxItems = [
  {
    id: "1",
    from: { name: "GitHub" },
    subject: "[Chaos UI] PR #234 合并通知",
    preview: "您的 Pull Request 已被合并到 main 分支",
...
const testimonials = [
  {
    quote: "Chaos UI 让我们的设计系统迭代速度提升了 3 倍。组件质量令人惊艳。",
    author: { name: "张明", role: "前端架构师", company: "Acme Corp" },
    rating: 5,
  },
  {
    quote: "可访问性考虑得非常周到，暗色模式下的色彩对比度是行业最佳实践。",
    author: { name: "李华", role: "设计师", company: "Design Studio" },
    rating: 5,
  },
  {
    quote: "Storybook 集成做得非常出色，组件文档化几乎不需要额外工作。",
    author: { name: "王芳", role: "技术负责人", company: "Tech Inc" },
    rating: 4,
  },
];
...
const faqItems = [
  {
    q: "如何开始使用 Chaos UI？",
    a: "通过 pnpm 安装依赖，参考 Getting Started 文档即可。",
  },
  { q: "支持哪些框架版本？", a: "支持 React 19+，Next.js 15+。" },
  { q: "如何定制主题？", a: "通过 CSS 变量和 Tailwind 配置调整。" },
  {
    q: "是否提供 TypeScript 类型？",
    a: "是，所有组件都有完整的 TypeScript 类型定义。",
  },
...
export const SignUpExample: Story = {
  render: () => (
    <div className="bg-card max-w-md rounded-md border p-6">
      ...
      <SignUpForm
        onSubmit={(v) => console.info("signup", v)}
        signinHref="#signin"
      />
    </div>
  ),
      ...
      <div className="max-w-md">
        <OnboardingChecklist
          ...
          onToggle={(id, completed) =>
            setDone((prev) =>
              completed ? [...prev, id] : prev.filter((d) => d !== id),
            )
          }
        />
      </div>
      ...
      <section>
        ...
        <div className="bg-card max-w-md rounded-md border p-6">
          <SignInForm
            ...
            ssoProviders={[{ id: "github", label: "GitHub" }]}
          />
        </div>
      </section>
      <section>
