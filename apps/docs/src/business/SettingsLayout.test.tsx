...
import { SettingsLayout, isNavGroup, isNavSubGroup } from "@/components/business/settings-layout"
import type { SettingsNavEntry, SettingsNavItem, SettingsNavSubGroup } from "@/components/business/settings-layout"

describe("SettingsLayout", () => {
  it("renders all flat nav items", () => {
    render(
      <SettingsLayout
        nav={[
          { key: "a", label: "账户" },
  ...
  it("calls onNavChange when flat nav item clicked", () => {
    const handle = vi.fn()
    render(
      <SettingsLayout
        nav={[
  ...
  it("applies data-slot attribute", () => {
    const { container } = render(
      <SettingsLayout nav={[]} active="a" sections={[]} />,
    )
    expect(container.querySelector('[data-slot="settings-layout"]')).toBeInTheDocument()
  })

  it("renders group label and its children when defaultOpen", () => {
    render(
      <SettingsLayout
        nav={[
          {
  ...
  it("renders sub-group (3rd level) items when defaultOpen", () => {
    render(
      <SettingsLayout
        nav={[
          {
  ...
  it("calls onNavChange when 3rd-level item clicked", () => {
    const handle = vi.fn()
    render(
      <SettingsLayout
        nav={[
  ...
  it("toggles group open/close on click", () => {
    render(
      <SettingsLayout
        nav={[
          {
