...
describe("PublicLayout", () => {
  ...
  it("renders nav links for each nav item", () => {
    ...
    const header = screen.getByRole("banner")
    expect(within(header).getByRole("link", { name: "Product" })).toHaveAttribute("href", "/product")
  ...
  it("does not render a footer when none is provided", () => {
    ...
    expect(container.querySelector('[data-slot="public-layout"] > footer')).toBeNull()
  })
  ...
  it("applies the public-layout slot and merges custom className", () => {
    ...
    const el = container.querySelector('[data-slot="public-layout"]') as HTMLElement
