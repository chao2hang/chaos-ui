...
describe("AppShell", () => {
  ...
  it("toggles the sidebar collapsed state when the desktop toggle is clicked", () => {
    ...
    const shell = screen.getByText("body").closest('[data-slot="app-shell"]') as HTMLElement
  ...
  it("opens and closes the mobile sidebar when the mobile toggle is clicked", () => {
    ...
    const sidebar = screen.getByText("navigation").closest("aside") as HTMLElement
    expect(sidebar.className).toContain("translate-x-0")

    fireEvent.click(toggle)
    const sidebarAfter = screen.getByText("navigation").closest("aside") as HTMLElement
  ...
  it("starts collapsed when defaultCollapsed is true", () => {
    ...
    const shell = screen.getByText("body").closest('[data-slot="app-shell"]') as HTMLElement
    expect(shell.getAttribute("data-sidebar-collapsed")).toBe("true")
    expect(within(shell).getByRole("button", { name: "展开侧栏" })).toBeInTheDocument()
  })
})
