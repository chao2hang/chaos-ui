...
describe("Carousel", () => {
  ...
  it("navigates between slides when the next/prev buttons are clicked", () => {
    ...
    fireEvent.click(container.querySelector('[data-slot="carousel-next"]') as HTMLElement)
    expect(onIndexChange).toHaveBeenLastCalledWith(1)
    fireEvent.click(container.querySelector('[data-slot="carousel-previous"]') as HTMLElement)
  ...
  it("jumps to a slide when a dot is clicked", () => {
    ...
    const dotButtons = container.querySelectorAll('[data-slot="carousel-dots"] button')
    expect(dotButtons).toHaveLength(3)
    expect((dotButtons[0] as HTMLButtonElement).getAttribute("aria-label")).toBe("Go to slide 1")
