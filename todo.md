...
**1.1 权限/反馈(4)**:`permission-wrapper` `global-loading` `notification-bell` `edit-toolbar`
...
**1.6 打印/输出(9)**:`bill-print-template` `barcode-display` `qrcode-display` `import-dialog` `template-download` `import-error-table` `batch-print-dialog` `print-template-builder` `print-button` `print-service`
**1.7 附件管理(6)**:`attachment-uploader` `attachment-list` `attachment-preview` `image-gallery` `file-card` `paste-upload`
**1.8 编号规则(2)**:`sequence-preview` `sequence-input`
...
**1.10 体验增强(8)**:`feature-tour` `keyboard-shortcut` `fullscreen-toggle` `theme-switcher` `preference-panel` `image-viewer` `dock-panel` `tab-pin`
**1.11 权限补充(2)**:`with-permission` `permission-button`
...
- [ ] AI 规则文件矩阵:AGENTS.md/CLAUDE.md/.cursorrules/CONVENTIONS.md/ARCHITECTURE.md/COMPONENT_INDEX.md/HOOKS_INDEX.md/LIB_INDEX.md
