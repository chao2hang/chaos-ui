...
**1.2 单据/审核(13)**:`master-edit-template` `master-list-template` `photo-audit` `operation-log` `bill-todo-list` `serial-number-manager` `dynamic-form-builder` `flow-tracker` `rebut-node-select` `approval-flow` `approval-action-bar` `bill-timeline` `oa-bridge`
**1.3 图表族(15,基于 recharts 对标 Tremor)**:`chart-suite` `funnel-chart` `gantt-chart` `calendar-view` `resource-schedule` `timeline-view` `heatmap-chart` `map-chart` `dashboard-canvas` `bar-list` `delta-bar` `category-bar` `callout` `badge-delta` `tracking` `chart-card` `stat-card-with-sparkline` `donut-card`
**1.4 browse picker 工厂(10)**:`customer-browse` `product-browse` `sales-order-browse` `writeoff-browse` `price-adjust-browse` `fee-type-browse` `shipping-way-browse` `warehouse-browse` `company-browse` `city-browse`
**1.5 财务/统计(17)**:`tax-detail-table` `writeoff-flow` `budget-overview` `target-progress` `inventory-snapshot` `invoice-summary` `account-balance` `reconciliation-line-editor` `sales-target-editor` `marketing-activity-form` `policy-line-editor` `application-form` `reconciliation-summary` `settlement-status-tag` `sign-action-button` `invoice-manager` `payment-schedule`
**1.6 打印/输出(9)**:`bill-print-template` `barcode-display` `qrcode-display` `import-dialog` `template-download` `import-error-table` `batch-print-dialog` `print-template-builder` `print-button` `print-service` ✅ 部分完成(barcode-display, qrcode-display, import-dialog, print-button 已建)
**1.7 附件管理(6)**:`attachment-uploader` `attachment-list` `attachment-preview` `image-gallery` `file-card` `paste-upload`
**1.8 编号规则(2)**:`sequence-preview` `sequence-input` ✅ 已完成
**1.9 仪表盘/看板补充(8)**:`task-list-table` `quick-entry-grid` `announcement-card` `performance-rank-table` `pool-tracker-table` `async-task-trigger` `task-progress` `task-history`
...
**1.12 聊天/AI 体系(28,P1)**:`chat-shell` `chat-sidebar` `chat-header` `chat-message` `chat-message-group` `chat-message-actions` `chat-thinking-block` `chat-tool-call-block` `chat-code-block` `chat-markdown-renderer` `chat-card-message` `chat-image-gallery` `chat-voice-message` `chat-input` `chat-input-toolbar` `chat-mention-picker` `chat-command-menu` `chat-streaming-text` `chat-suggest-replies` `chat-context-panel` `chat-model-switcher` `chat-agent-status` `chat-artifact-panel` `chat-feedback` `chat-conversation` `chat-conversation-search` `chat-branch` `chat-shared-link`(依赖 Shiki/Prism/KaTeX/Mermaid)
**1.13 低代码设计器(5,P2)**:`form-designer` `form-designer-runtime` `workflow-designer` `workflow-preview` `rule-editor`(依赖 reactflow)
**1.14 移动端补充(10,P2)**:`mobile-page-shell` `mobile-action-sheet` `mobile-picker` `mobile-camera` `mobile-qrcode-scanner` `mobile-signature` `mobile-geolocation` `mobile-infinite-scroll` `mobile-tab-bar` `mobile-list-item`
