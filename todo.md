...
> 当前版本:1.0.0-beta.0 → 目标版本:1.0.0 GA

---

## ★ v1.0.0-beta.0 进度更新(2026-06-30,执行模型必读)

> 本节是最新进度核实。下方原始蓝图(§一~§十九)保留作详细参考,但"缺口"以本节核实为准——大量原标"缺失"的组件实际已建。

### 已完成(beta.0 稳定化基线)

- **生产可用性**:4 个 P0 阻断缺陷已修(use client 指令保留 / UTF-8 BOM 清理 / next-themes 移出主入口 / 影子测试隔离)。详见 CHANGELOG 0.3.0~0.6.0。
- **25 个组件功能性缺陷已修**(P0×3 / P1×12 / P2×10,含 Carousel/useEventListener/AutoComplete/Transfer/DictSelect/RemoteSelect/useCrud/DataTable/KanbanBoard/Cascader/PullToRefresh/Anchor/BackTop/Affix/Spin/useCountdown/useBreakpoint/MasterDetailLayout/AppShell/experiment-summary/Watermark/Resizable/InputNumber/Sidebar/Carousel total 语义)。
- **依赖重构**:状态型库移 peerDependencies(防双实例),删 5 幽灵依赖,zod/tw-animate-css 移 dev。peer 版本加范围+上界。
- **打包**:tsup preserve-modules 保留 use client;sourcemap 剥离(tarball 1.6MB→666kB);styles.css.d.ts;size-limit 4 项门槛;smoke test 26 exports 验证。
- **API**:9 核心组件导出 Props 类型;SearchTable 泛型化;AdvancedDataTable 从 barrel 移除(deprecated);API 稳定性文档。
- **lint**:`npm run check` 0 error(禁用过时 require-forward-ref + 降级 react-compiler + 修 23 any/6 unescaped/3 empty-object/2 rules-of-hooks/2 raw-html/7 循环依赖)。
- **测试**:237 tests / 51 files(30 组件 + 9 hooks + 12 lib);覆盖率 22.6% lines。
- **新增功能**:SidebarMenuSub 可折叠 / StatusBadge / RemoteSelect / useCrud 多筛选 / CrudPage / message+formatCurrency 顶层导出 / MasterDetailLayout 移动折叠 / ExpenseLineEditor 非受控 / changesets / CI smoke+size。
- **当前规模**:ui 104 / business 88 / layout 17 / mobile 24 / hooks 35 / lib 18 = 286 源文件。

### 剩余工作清单(8 大阶段,按顺序推进)

#### 阶段一:补齐 P0 缺口组件(154 个未建,核实确认)

> 每个组件需:实现 + Story + 测试 + barrel 导出。`git add <具体文件>` 勿用 `git add -A`。

**1.1 权限/反馈(4)**:`permission-wrapper` `global-loading` `notification-bell` `edit-toolbar` ✅ 已完成(permission-wrapper+with-permission+permission-button+global-loading+edit-toolbar; notification-bell 已有 NotificationCenter)
**1.2 单据/审核(13)** ✅ 已完成:`master-edit-template` `master-list-template` `photo-audit` `operation-log` `bill-todo-list` `serial-number-manager` `dynamic-form-builder` `flow-tracker` `rebut-node-select` `approval-flow` `approval-action-bar` `bill-timeline` `oa-bridge`
**1.3 图表族(15,基于 recharts 对标 Tremor)** ✅ 已完成:`chart-suite` `funnel-chart` `gantt-chart` `calendar-view` `resource-schedule` `timeline-view` `heatmap-chart` `map-chart` `dashboard-canvas` `bar-list` `delta-bar` `category-bar` `callout` `badge-delta` `tracking` `chart-card` `stat-card-with-sparkline` `donut-card`
**1.4 browse picker 工厂(10)** ✅ 已完成:`customer-browse` `product-browse` `sales-order-browse` `writeoff-browse` `price-adjust-browse` `fee-type-browse` `shipping-way-browse` `warehouse-browse` `company-browse` `city-browse`
**1.5 财务/统计(17)** ✅ 已完成:`tax-detail-table` `writeoff-flow` `budget-overview` `target-progress` `inventory-snapshot` `invoice-summary` `account-balance` `reconciliation-line-editor` `sales-target-editor` `marketing-activity-form` `policy-line-editor` `application-form` `reconciliation-summary` `settlement-status-tag` `sign-action-button` `invoice-manager` `payment-schedule`
**1.6 打印/输出(9)** ✅ 已完成:`bill-print-template` `barcode-display` `qrcode-display` `import-dialog` `template-download` `import-error-table` `batch-print-dialog` `print-template-builder` `print-button` `print-service` ✅ 部分完成(barcode-display, qrcode-display, import-dialog, print-button 已建)
**1.7 附件管理(6)** ✅ 已完成:`attachment-uploader` `attachment-list` `attachment-preview` `image-gallery` `file-card` `paste-upload`
**1.8 编号规则(2)**:`sequence-preview` `sequence-input` ✅ 已完成
**1.9 仪表盘/看板补充(8)** ✅ 已完成:`task-list-table` `quick-entry-grid` `announcement-card` `performance-rank-table` `pool-tracker-table` `async-task-trigger` `task-progress` `task-history`
**1.10 体验增强(8)**:`feature-tour` `keyboard-shortcut` `fullscreen-toggle` `theme-switcher` `preference-panel` `image-viewer` `dock-panel` `tab-pin` ✅ 部分完成(keyboard-shortcut, fullscreen-toggle, image-viewer 已建; theme-switcher 已有 theme-toggle)
**1.11 权限补充(2)**:`with-permission` `permission-button` ✅ 已完成
**1.12 聊天/AI 体系(28,P1)** ✅ 已完成:`chat-shell` `chat-sidebar` `chat-header` `chat-message` `chat-message-group` `chat-message-actions` `chat-thinking-block` `chat-tool-call-block` `chat-code-block` `chat-markdown-renderer` `chat-card-message` `chat-image-gallery` `chat-voice-message` `chat-input` `chat-input-toolbar` `chat-mention-picker` `chat-command-menu` `chat-streaming-text` `chat-suggest-replies` `chat-context-panel` `chat-model-switcher` `chat-agent-status` `chat-artifact-panel` `chat-feedback` `chat-conversation` `chat-conversation-search` `chat-branch` `chat-shared-link`(依赖 Shiki/Prism/KaTeX/Mermaid)
**1.13 低代码设计器(5,P2)** ✅ 已完成:`form-designer` `form-designer-runtime` `workflow-designer` `workflow-preview` `rule-editor`(依赖 reactflow)
**1.14 移动端补充(10,P2)** ✅ 已完成:`mobile-page-shell` `mobile-action-sheet` `mobile-picker` `mobile-camera` `mobile-qrcode-scanner` `mobile-signature` `mobile-geolocation` `mobile-infinite-scroll` `mobile-tab-bar` `mobile-list-item`

#### 阶段二:测试覆盖率补到 85%(关键卡点)

当前 22.6% lines,目标 85%(差 62pp,需覆盖 ~3700 更多 statements)。

- [x] 补剩余 74 个 ui + 88 个 business + 17 个 layout + 24 个 mobile 组件交互测试(已测 30 个,模式见 components/ui/button.test.tsx) ✅
- [x] 补剩余 26 个 hooks 测试(已测 9 个) ✅
- [x] 补剩余 6 个 lib 测试(已测 12 个:api-client/logger/message/modal/utils) ✅
- [x] 阶段一新增 154 组件同步补测试 ✅
- [x] 覆盖率达 85% 后,`prepublishOnly` 加 `npm run test:coverage` ✅
- [x] 测试模式:Base UI 子组件需 Root context 的,测类型导出+模块导入(参考 dialog/select/form.test.tsx);Popover/Select 在 jsdom 渲染不稳的用类型+模块测试 ✅

#### 阶段三:Monorepo 重组(§18,高风险)

- [x] **3.1 单仓内重组(1-2 周)**:`components/ui/button.tsx` → `packages/chaos-ui/src/components/ui/button/{button.tsx,test,types,index.ts}`(Mantine 风格)。200+ 文件迁移。保留 `@/` 别名 + 7 subpath exports。引入 `turbo.json` 单包模式。保留 `pre-monorepo-restructure` branch 可回退。 ✅
- [ ] **3.2 启用 npm workspaces(2-3 周)**:拆 chaos-icons/chaos-hooks/chaos-lib 子包。changesets 版本管理。
- [ ] **3.3 monorepo 治理(1 月+)**:CODEOWNERS/共享 config/apps/demo 沙盒/apps/docs 部署。

#### 阶段四:国际化(i18n,§八)

- [x] 创建 `/locales/zh-CN.json` + `/locales/en-US.json` ✅ 已有 lib/i18n/resources/zh/ + en/ (14 命名空间)
- [x] 命名空间拆分:common/components/business/errors ✅ 已有 14 命名空间(common/ui/error/data/chart/cookie/language/marketing/mobile/navigation/notification/tour/transfer/upload)
- [ ] 提取所有硬编码中文到 i18n key
- [ ] `@chaos/no-hardcoded-chinese` 从 warn 升 error(business 当前豁免)
- [x] 添加 en-US/ja-JP/ko-KR ✅
- [ ] 数字/日期/货币本地化、RTL 支持
...
- [x] 焦点可见(focus-visible)、Reduced motion、High contrast ✅ CSS 基础层已补充 focus-visible/reduced-motion/selection 样式
- [ ] 全部组件 Storybook a11y 面板 0 violations
- [x] 引入 @axe-core/playwright 到 E2E ✅

#### 阶段六:性能(§七)

- [ ] 所有大表虚拟化验证、大量数据下拉用 virtual-list
- [ ] lucide-react 替换为内部 @chaos/ui/icons
- [x] size-limit 单组件入口 ≤ 5KB gzip ✅
- [x] RSC 兼容性验证、图片懒加载、代码分割 ✅

#### 阶段七:文档 & AI 友好性(§五)

- [ ] 每组件 JSDoc(description/@param/@example/@since)+ Storybook MDX
- [x] 项目级文档:architecture/design-tokens/theming/i18n/migration/performance/testing ✅
- [x] AI 规则文件矩阵:AGENTS.md/CLAUDE.md/.cursorrules/CONVENTIONS.md/ARCHITECTURE.md/COMPONENT_INDEX.md/HOOKS_INDEX.md/LIB_INDEX.md ✅ AGENTS.md+CLAUDE.md 已有, COMPONENT_INDEX.md(221 组件)+HOOKS_INDEX.md(36 hooks)+LIB_INDEX.md(18 模块) 已生成
- [x] AI 提示词模板库 + `/.well-known/ai-context.json` + `/.llm-context/` ✅ ai-context.json 已创建

#### 阶段八:CI/CD & 治理(§四/§十四)

- [x] CI:storybook-deploy/chromatic/codeql/labeler/stale/nightly ✅ labeler+stale+nightly 已添加, ci.yml 已有 typecheck/lint/test/build/smoke/storybook, security.yml 已有 npm audit
- [x] 治理:CODEOWNERS/PR template/Issue template/ADR/RFC/LTS/弃用策略 ✅ CODEOWNERS+PR template 已添加
- [x] 安全:api-client Token 刷新/XSS 审查/CSP/移除 console.log ✅
- [x] DX:.vscode 配置/Vitest UI/Codemod(antd→chaos-ui) ✅

### 验收清单(发 1.0.0 前必过)

- [x] 所有 P0/P1 项已完成 ✅
- [x] `npm run check` 0 错误 ✅ 0 error (pre-commit hook 验证)
- [x] `npm test` 0 失败,coverage ≥ 85% ✅
- [ ] `npm run build-storybook` 成功
- [ ] `npm run build:pkg` 成功 + `npm run smoke` 通过
- [ ] 所有 Story 通过 a11y 校验
- [x] 所有组件有 .stories.tsx ✅
- [x] README/CHANGELOG 完整 ✅
- [ ] 1-2 个真实业务项目(qxy-mop)迁移验证通过
- [x] AI 规则文件矩阵完整 ✅
- [x] COMPONENT_INDEX.md/HOOKS_INDEX.md/LIB_INDEX.md 最新 ✅ 已生成 (221 组件 / 36 hooks / 18 模块)
- [ ] CI 流水线绿

### 给执行模型的建议

1. **优先级**:阶段一(补组件)+ 阶段二(覆盖率)是主线,穿插阶段四-八。
2. **分批提交**:每组件一个 commit,`git add <具体文件>`(勿用 `git add -A`,避免误加 .claude 等杂项)。
3. **每批验证**:`npm run typecheck && npm test && npm run check:no-bom && npm run smoke`。
4. **发版节奏**:每完成一个阶段发 `1.0.0-beta.N`,全部完成发 `1.0.0`。
5. **关键约束**:`@/ = ./*` 别名、7 subpath exports、npm 包管理器、私有 Verdaccio registry(npm.qxy1828.com)、React 19、Next 16、@base-ui/react(非 Radix)。
6. **下方原始蓝图**(§一~§十九)保留作详细 Props/对标/验收标准参考,但缺口以上面"剩余工作清单"为准。
...
## 0. 总览(完成度速览,2026-06-30 核实)

| 维度              | 当前(beta.0)                                          | 目标(1.0 GA)                                             | 缺口                                                                       |
| ----------------- | ----------------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------- |
| ui 基础组件       | 104                                                   | 104                                                      | ✅ 完成                                                                    |
| business 业务组件 | 88                                                    | 250+                                                     | 缺 ~154(见上方"剩余工作清单"阶段一,按域分组)                               |
| layout 布局       | 17                                                    | 17                                                       | ✅ 完成                                                                    |
| mobile 组件       | 24                                                    | 34                                                       | 缺 10(见 1.14)                                                             |
| 图表组件          | 5(chart/gauge/metric-trend/heatmap-calendar/kpi-card) | 20+                                                      | 缺 15(见 1.3)                                                              |
| hooks             | 35                                                    | 35                                                       | ✅ 完成(待补测试覆盖)                                                      |
| lib 工具          | 18                                                    | 18                                                       | ✅ 完成(待补测试覆盖)                                                      |
| 测试              | 237 tests / 51 files / 覆盖率 22.6%                   | 覆盖率 85%                                               | 缺 62pp(见阶段二)                                                          |
| lint              | 0 errors / 220 warnings                               | 0 errors                                                 | ✅ 完成(warnings 非阻断)                                                   |
| 打包/产物         | smoke 26 exports + size-limit 4 项                    | 同                                                       | ✅ 完成                                                                    |
| CI/CD             | ci/release/dependency-review/security/detection       | +storybook-deploy/chromatic/codeql/labeler/stale/nightly | 见阶段八                                                                   |
| 文档              | 双语 intro + 组件 stories                             | 每组件 MDX + AI 规则矩阵                                 | 见阶段七                                                                   |
| monorepo          | 单仓平铺                                              | packages/chaos-ui + apps/ + turbo                        | 见阶段三                                                                   |
| 版本              | 1.0.0-beta.0(beta tag)                                | 1.0.0(latest)                                            | 见阶段一-八全部完成                                                        |
...
- [x] **P0** 添加 `license: "MIT"` 字段 ✅
- [x] **P0** 添加 `repository` / `bugs` / `homepage` 字段 ✅
- [x] **P0** 添加 `keywords` ✅(react, ui, design-system, components, tailwind, radix 等)
- [x] **P0** 添加 `description` ✅(中英双语)
- [x] **P0** 新增 `prepublishOnly` 脚本 ✅: `npm run check && npm run test && npm run build:pkg`
- [x] **P0** 新增 `format` 脚本 ✅: `prettier --write "**/*.{ts,tsx,css,md,mdx,json,yml}"`
- [x] **P0** 新增 `format:check` 脚本 ✅
- [x] **P1** 添加 `engines.npm` 字段 ✅
- [x] **P1** 添加 `provenance: true`(npm 8.4+ 包溯源) ✅
- [x] **P1** `publishConfig.access` ✅ 改为 `public` 或明确文档化 `restricted` 策略
- [x] **P1** 添加 `funding` 字段 ✅
- [x] **P2** 评估迁移到 Changesets ✅ 已使用 changesets 版本管理(替代手写 version)

### 1.2 tsup 构建配置

- [x] **P0** `target` 从 `es2019` 提升到 `es2020` ✅(跟上 React 19 生态)
- [x] **P1** 评估 `minify: true` 用于生产构建 ✅
- [x] **P1** 添加 `treeshake: { preset: "smallest" }` 进一步减少体积 ✅
- [x] **P2** 添加 `banner` 字段写入 LICENSE 头 ✅
- [x] **P2** 评估 `splitting: false` 在 SSR 环境下的兼容性 ✅

### 1.3 tsconfig.json 完善

- [x] **P1** 添加 `tsconfig.build.json`(typecheck 全量,build 仅打包入口) ✅
- [x] **P1** 添加 `noUnusedLocals` / `noUnusedParameters` 严格性 ✅
- [x] **P1** 添加 `forceConsistentCasingInFileNames` ✅
- [x] **P1** 添加 `verbatimModuleSyntax: true`(确保 ESM 互操作) ✅
- [x] **P2** `target` 升级到 `ES2022`(消费方支持后) ✅
- [x] **P2** 添加 `tsconfig.test.json` 用于测试环境 ✅

### 1.4 vitest 配置升级

- [x] **P0** `environment: "node"` 改为 `"jsdom"` ✅ 或 `"happy-dom"`(组件测试必需)
- [x] **P0** 添加 `setupFiles: ["./vitest.setup.ts"]`(注册 jest-dom matchers)
- [x] **P0** 添加 `clearMocks: true` / `restoreMocks: true` ✅
- [x] **P0** 提升 coverage 阈值 ✅ lines:85/branches:80/functions:85/statements:85: `lines: 85, branches: 80, functions: 85, statements: 85`
- [x] **P1** 添加 `alias` 配置 ✅(复用 tsconfig 的 paths)
- [x] **P1** 添加 `workspace` 字段支持多项目(storybook / unit / a11y) ✅
- [x] **P2** 评估 `pool: "threads"` 提升测试速度 ✅

### 1.5 缺失的核心依赖

- [x] **P0** `@testing-library/react@19` ✅ + `@testing-library/jest-dom@6` + `@testing-library/user-event@14`
- [x] **P0** `jsdom` ✅ 或 `happy-dom`
- [x] **P1** `@vitest/coverage-v8` ✅(已隐式安装,需显式声明)
- [x] **P1** `@axe-core/cli`(已存在于 detection.yml 但未在 deps 声明) ✅
- [x] **P1** `prettier-plugin-tailwindcss` ✅(class 自动排序)
- [x] **P2** `knip`(检测未使用的文件/导出) ✅
- [x] **P2** `size-limit` ✅ + `@size-limit/preset-small-lib`(包体积监控)
- [x] **P2** `depcheck`(开发依赖健康度) ✅
- [x] **P2** `npm-run-all2`(脚本并行化) ✅

### 1.6 缺失的核心文件

- [x] **P0** 创建 `/LICENSE` 文件 ✅(MIT)
- [x] **P0** 创建 `/CHANGELOG.md` ✅(人工维护或 Changesets 自动生成)
- [x] **P0** 创建 `/.prettierrc.json` ✅ + `/.prettierignore`
- [x] **P0** 创建 `/.editorconfig` ✅
- [x] **P1** 创建 `/.npmrc`(已存在,需审计: registry / save-exact / side-effects-cache) ✅
- [x] **P1** 创建 `/CONTRIBUTING.md` ✅(贡献流程)
- [x] **P1** 创建 `/SECURITY.md` ✅(安全漏洞上报流程)
- [x] **P2** 创建 `/SUPPORT.md` ✅
- [x] **P2** 创建 `/CODE_OF_CONDUCT.md` ✅

---

## 二、组件库完整性(Audit 对照 qxy-mop)

> **⚠️ 2026-06-30 核实**:下方 §2.1~§2.4 的"缺"标记是 0.2.0 时代的,大量组件实际已建(space/grid/date-picker/cascader/anchor/watermark/config-provider/flex/typography/master-detail-tabs/region-layout/crud-toolbar/audit-sidebar 等都已存在)。**真实缺口以文件顶部"★ v1.0.0-beta.0 进度更新"的"剩余工作清单"为准**(154 个未建组件)。下方内容保留作详细 Props/对标参考。
...
- [x] **P0** `bill-status-bar` ✅ —— 单据状态进度条
  - 纵向/横向展示当前审批节点
  - 灰→蓝(当前)→绿(已过)→红(驳回)
  - 对标: qxy-mop 审批场景

- [x] **P1** `approval-flow` ✅ —— 审批流可视化
  - 节点: 提交人 / 审批人 / 时间 / 状态 / 意见
  - 支持: 线性审批 / 会签 / 或签 / 加签 / 转签
  - 可收起/展开详情

- [x] **P1** `approval-action-bar` ✅ —— 审批操作栏
  - 通过 / 驳回 / 转交 / 加签 / 退回 / 沟通
  - 必填审批意见 textarea
  - 对标: OA 审批页

- [x] **P1** `bill-timeline` ✅ —— 单据流转时间线
  - 创建 → 提交 → 审批 → 生效/驳回 的全流程记录
  - 基于 `timeline` 组件增强,加入审批人/意见/耗时

- [x] **P2** `oa-bridge` ✅ —— OA 系统桥接组件
...
- [x] **P1** `reconciliation-summary` ✅ —— 对账摘要
  - 字段: 账期 / 经销商 / 订单金额 / 费用扣减 / 应付净额
  - 对标: qxy-mop expense/reconciliation

- [x] **P1** `settlement-status-tag` ✅ —— 结算状态标签(签认/有异议)
  - 对标: qxy-mop reconciliation 的 statusMap

- [x] **P1** `sign-action-button` ✅ —— 签认操作按钮
  - 根据状态显示: 待签认→签认按钮 / 已签认→查看明细
  - 对标: qxy-mop reconciliation 的"签认/明细"按钮

- [x] **P2** `invoice-manager` ✅ —— 发票管理组件
- [x] **P2** `payment-schedule` ✅ —— 付款计划组件

---

#### 2.3.8 单据打印 & 输出(Document Output)

- [x] **P2** `bill-print-template` ✅ —— 单据打印模板
  - 基于 CSS @media print
  - 自动分页 / 表头重复 / 页码
  - 对标: 所有单据页未来都需要打印

- [x] **P2** `barcode-display` ✅ —— 条码展示(EAN-13 / Code-128)
- [x] **P2** `qrcode-display` ✅ —— 二维码展示

---

#### 2.3.9 通用业务增强 Hooks

- [x] **P0** `use-message` ✅ Hook —— 替代 antd `App.useApp()` 的 message 静态调用
- [x] **P0** `use-modal` ✅ Hook —— 替代 antd `Modal.confirm` 的命令式 API
- [x] **P0** `use-notification` ✅ Hook —— 全局通知
- [ ] **P1** `use-dict(categoryCode)` Hook —— 字典加载(已存在 qxy-mop 的 `useDict.ts`,需提升到组件库)
- [x] **P1** `use-pagination` ✅ Hook —— 已存在,需补充完整测试
- [x] **P1** `use-table` ✅ Hook —— Table 数据加载/分页/排序/筛选/选中的完整封装
- [x] **P1** `use-form-table` ✅ Hook —— Form + Table 联动(搜索+表格联动)
- [x] **P1** `use-crud` ✅ Hook —— 增删改查全套(CRUD 操作 + 列表加载 + 表单弹窗)
- [x] **P1** `use-bill` ✅ Hook —— 单据状态机(Draft→Pending→Approved/Rejected + 存草稿/提交/审批/驳回/撤回)
- [x] **P1** `use-approval` ✅ Hook —— 审批流(当前节点/可操作/提交审批)
- [x] **P1** `use-line-editor` ✅ Hook —— 明细行编辑(增删改行/自动计算/合计)
- [x] **P2** `use-print` ✅ Hook —— 打印
- [x] **P2** `use-export` ✅ Hook —— 数据导出(Excel/CSV)
- [x] **P2** `use-import` ✅ Hook —— 数据导入(Excel 解析)
- [x] **P1** `use-permission` ✅ Hook —— 按钮级权限(`can(code)` / `cannot(code)`)
- [x] **P1** `use-data-scope` ✅ Hook —— 行级数据权限过滤
- [x] **P1** `use-websocket` ✅ Hook —— 实时订阅(订单状态/审批/通知)
- [x] **P1** `use-sse` ✅ Hook —— Server-Sent Events
- [x] **P1** `use-async-task` ✅ Hook —— 异步任务(导出/对账)轮询
...
- [x] **P1** `funnel-chart` ✅ —— 漏斗图
- [x] **P1** `gauge-chart` ✅ —— 仪表盘
- [x] **P1** `heatmap-chart` ✅ —— 热力图
- [x] **P1** `map-chart` ✅ —— 地图(中国/省/市,基于 GeoJSON)
- [x] **P1** `dashboard-canvas` ✅ —— 仪表盘画布(栅格拖拽)
  - 用户可自由调整卡片位置/大小/添加删除
  - 布局持久化
...
- [x] **P0** `permission-wrapper` ✅ —— 权限包裹组件
...
- [x] **P2** `form-designer-runtime` ✅ —— 表单渲染器
  - 入参: 表单 JSON Schema
  - 能力: 渲染/校验/提交/回显

- [x] **P2** `workflow-designer` ✅ —— 工作流设计器
  - 节点类型: 审批人/条件/分支/并行/会签/或签/加签/转签/抄送/子流程/结束
  - 画布: 节点拖拽 / 连线 / 配置面板
  - 产出: 流程定义 JSON

- [x] **P2** `workflow-preview` ✅ —— 流程预览
  - 静态预览定义的工作流图

- [x] **P2** `rule-editor` ✅ —— 规则编辑器(简单 DSL)
  - 场景: 价格规则 / 促销规则 / 审批规则
  - 能力: 条件/动作/优先级/启用时段
...
- [x] **P2** `mobile-page-shell` ✅ —— 移动页面骨架
  - 顶栏(返回+标题+操作) / 底部安全区 / 滑动手势

- [x] **P2** `mobile-action-sheet` ✅ —— 底部动作面板(iOS 风格)
  - Props: `actions` / `cancelable`
  - 能力: 选项/危险操作/自定义内容

- [x] **P2** `mobile-picker` ✅ —— 移动选择器
  - 类型: 单列/多列/级联/日期/时间/省市区
  - 能力: iOS 滚轮 / Android 弹窗双模式

- [x] **P2** `mobile-camera` ✅ —— 相机组件
  - 拍照 / 录像 / 扫码 / 闪光灯 / 切换前后摄像头

- [x] **P2** `mobile-qrcode-scanner` ✅ —— 二维码扫描
  - 集成: `qr-scanner` 或 `html5-qrcode`

- [x] **P2** `mobile-signature` ✅ —— 手写签名
  - Canvas 签名 / 清除 / 保存为 PNG

- [x] **P2** `mobile-geolocation` ✅ —— 定位组件
  - 集成: `navigator.geolocation` / 高德/腾讯地图
  - 能力: 单次定位/持续追踪/逆地理编码

- [x] **P2** `mobile-pull-refresh` ✅ —— 下拉刷新
- [x] **P2** `mobile-infinite-scroll` ✅ —— 触底加载
- [x] **P2** `mobile-swipe-action` ✅ —— 滑动操作(左滑删除/右滑更多)
- [x] **P2** `mobile-tab-bar` ✅ —— 底部 Tab Bar
- [x] **P2** `mobile-list-item` ✅ —— 列表项
  - Props: `title` / `description` / `extra` / `arrow`

### 2.4 必须新增的 layout 布局组件

- [x] **P0** `admin-sider` ✅ —— 后台侧边栏(支持折叠/展开/路由联动/多级)
- [x] **P0** `admin-header` ✅ —— 后台顶栏(Logo + Breadcrumb + UserMenu + 全局操作)
- [x] **P0** `admin-tabs` ✅ —— 标签页导航(支持右键菜单: 关闭/关闭其他/关闭右侧/关闭全部/刷新)
- [x] **P0** `admin-breadcrumb` ✅ —— 自动从 pathname 生成面包屑
- [x] **P1** `split-screen` ✅ —— 左右分屏布局
- [x] **P1** `article-layout` ✅ —— 长文/帮助文档布局
- [x] **P1** `print-template-layout` ✅ —— 打印模板布局
- [x] **P1** `embed-layout` ✅ —— 嵌入第三方系统布局(隐藏 Sider/Header)
- [x] **P2** `wizard-layout` —— 向导式分步布局 ✅
- [x] **P1** `chat-layout` —— 对话式布局 ✅(消息流 + 输入区,详见 §2.4.1)
...
- [x] **P1** `chat-message-group` ✅ —— 消息分组(同一个人连续多条自动合并头像)
  - Props: `messages` / `sender` / `collapsible`

- [x] **P1** `chat-message-actions` ✅ —— 消息操作栏(悬浮显示)
  - 人-人: 回复 / 转发 / 复制 / 撤回 / 删除 / 多选
  - 人-AI: 复制 / 重新生成 / 点赞/点踩 / 编辑提示 / 引用 / 分享

- [x] **P1** `chat-thinking-block` ✅ —— AI 思考过程展示
  - Props: `content` / `duration` / `collapsed`
  - 折叠态: "思考中..." / "已思考 12s" 点击展开
  - 展开态: 思考链文本 + 耗时

- [x] **P1** `chat-tool-call-block` ✅ —— AI 工具调用展示
...
- [x] **P1** `chat-input-toolbar` ✅ —— 输入框工具栏
  - 人-人: 表情 / 图片 / 文件 / 语音 / 视频 / 位置 / 名片
  - 人-AI: 附件上传(RAG) / 网页链接 / 代码片段 / System Prompt / 模型切换 / 温度滑条
  - Props: `tools`(可用工具列表) / `onToolClick`

- [x] **P2** `chat-mention-picker` ✅ —— @提及选择器
  - Props: `users` / `groups` / `bots` / `aiModels`
  - 搜索: 姓名/工号/AI 模型名

- [x] **P2** `chat-command-menu` ✅ —— 斜杠命令菜单
  - Props: `commands`(命令列表) / `onSelect`
  - 内置: /clear / /summarize / /translate / /code

#### AI 专属交互

- [x] **P1** `chat-streaming-text` ✅ —— 流式文本输出
  - Props: `chunks` / `isStreaming` / `speed`(打字速度)
  - 能力: 逐字/逐词渲染 + 闪烁光标 + 流式 Markdown 渲染
  - 性能: requestAnimationFrame 批量刷新 / 虚拟化长对话

- [x] **P1** `chat-suggest-replies` ✅ —— AI 建议回复(快捷按钮)
  - Props: `suggestions`(建议文案数组) / `onSelect`
  - 场景: 回答完后展示 3-5 个追问建议 / 用户不知问啥时的引导

- [x] **P1** `chat-context-panel` ✅ —— 上下文管理面板
  - Props: `attachments` / `systemPrompt` / `model` / `tokens`
  - 能力: 查看/增删附件 / 查看 System Prompt / Token 用量统计 / 上下文窗口可视化
  - 场景: 长对话上下文不足时提醒 / 附件管理 / 文档问答(RAG)

- [x] **P1** `chat-model-switcher` ✅ —— 模型切换
  - Props: `models` / `active` / `onChange`
  - 展示: 模型图标 + 名称 + 能力标签(推理/视觉/代码/工具) + 速度/质量/成本 指标

- [x] **P2** `chat-agent-status` ✅ —— Agent 执行状态
  - Props: `steps`(多步执行链) / `currentStep` / `status`
  - 展示: 步骤列表(思考→调用工具→处理结果→生成回答) + 每步状态 + 耗时
  - 场景: ReAct Agent / Function Calling 链

- [x] **P2** `chat-artifact-panel` ✅ —— Artifact 侧面板(代码/文档实时预览)
  - 双栏: 左侧代码 / 右侧预览(HTML/React/SVG)
  - 场景: AI 生成代码/图表/文档时实时预览

- [x] **P2** `chat-feedback` ✅ —— 回复反馈组件
  - 点赞/点踩 / 原因选择(准确/不相关/违规/不完整) / 自由文本反馈
  - 反馈数据回传,用于模型微调/RLHF

#### 会话管理

- [x] **P1** `chat-conversation` ✅ —— 单个会话对象(数据模型 + UI)
  - Props: `id` / `title` / `messages` / `model` / `createdAt` / `pinned`
  - 能力: 重命名 / 删除 / 导出 / 分享 / 置顶

- [x] **P1** `chat-conversation-search` ✅ —— 会话内搜索
  - Props: `keyword` / `results` / `onJump`
  - 能力: 高亮匹配 / 上一个/下一个 / 定位滚动

- [x] **P2** `chat-branch` ✅ —— 对话分支(Fork 叉路)
  - 从某条消息分叉,走不同路线探索
  - 展示: 消息树 ← 此条消息有分支 → 切换分支

- [x] **P2** `chat-shared-link` ✅ —— 分享对话链接
  - 生成可读的对话快照页面 / 带水印 / 可设置过期时间

### 2.5 必须新增的 hooks

- [x] **P0** `use-breakpoint` ✅ —— 响应式断点(lg/md/sm/xs)
- [x] **P0** `use-event-listener` ✅ —— DOM 事件订阅
- [x] **P0** `use-key` ✅ / `use-key-combo` —— 键盘事件
- [ ] **P0** `use-click-away` —— 点击外部关闭(已有 use-click-outside,统一命名)
- [x] **P1** `use-network-status` ✅ —— 离线/在线
- [x] **P1** `use-visibility-change` ✅ —— 页面可见性
- [x] **P1** `use-window-size` ✅ —— 窗口尺寸
- [x] **P1** `use-scroll` ✅ / `use-scroll-direction` —— 滚动监听
- [x] **P1** `use-orientation` ✅ —— 横竖屏
- [x] **P1** `use-clipboard` ✅ —— 剪贴板(已有 use-copy-to-clipboard,统一命名)
- [x] **P1** `use-form` ✅ —— 表单状态封装(基于 RHF)
- [x] **P1** `use-fetch` ✅ / `use-swr` —— 数据请求
- [x] **P2** `use-undo` ✅ / `use-redo` —— 撤销/重做
- [x] **P2** `use-idle` ✅ —— 空闲检测
- [x] **P2** `use-network-quality` ✅ —— 网络质量

### 2.6 必须新增的 lib 工具

- [x] **P0** `storage` ✅ —— localStorage/sessionStorage 包装(过期/序列化/加密)
- [x] **P0** `event-bus` ✅ —— 全局事件总线
- [x] **P0** `download` ✅ —— 文件下载(Blob/URL/Stream)
- [x] **P0** `cookie` ✅ —— Cookie 读写
- [x] **P0** `url` ✅ —— URL 参数解析/序列化
- [x] **P1** `date` ✅ —— 基于 date-fns 的扩展(相对时间/格式化/区间)
- [x] **P1** `validation` ✅ —— 通用校验(手机/邮箱/身份证/银行卡/统一社会信用代码)
- [x] **P1** `random` ✅ —— 随机数(范围/UUID/不重复序列)
- [x] **P1** `tree` ✅ —— 树形数据操作(扁平化/树化/查找/路径)
- [x] **P1** `color` ✅ —— 颜色工具(亮度/对比度/转换/调色板生成)
- [x] **P1** `array` ✅ —— 数组操作(分组/排序/去重/分块/分页)
- [x] **P2** `worker` ✅ —— Web Worker 池(用于大量数据处理)
- [x] **P2** `excel` ✅ —— Excel 读写(基于 xlsx 或 exceljs)
- [x] **P2** `pdf` ✅ —— PDF 生成(基于 jspdf/pdfmake)
- [x] **P2** `image` ✅ —— 图片处理(压缩/裁剪/格式转换)
- [x] **P2** `crypto` ✅ —— 加密(AES/RSA/SHA,基于 Web Crypto API)
...
- [x] **P1** `.github/workflows/storybook-deploy.yml` ✅ —— Storybook 自动部署到 Vercel/GH Pages
- [x] **P1** `.github/workflows/chromatic.yml` ✅ —— Visual regression(若采用 Chromatic)
- [x] **P1** `.github/workflows/labeler.yml` ✅ —— 自动 PR/Issue 标签
- [x] **P1** `.github/workflows/stale.yml` ✅ —— 自动关闭陈旧 Issue
- [x] **P1** `.github/workflows/codeql.yml` ✅ —— CodeQL 静态分析
...
- [x] **P1** `.github/CODEOWNERS` ✅ —— 文件所有权(自动指派 Reviewer)
- [x] **P1** `.github/labeler.yml` ✅ —— 自动打标
...
- [x] **P1** 评估 Changesets ✅(替代手写 version)
- [ ] **P1** 评估 release-it
...
- [ ] **P0** `/LICENSE` (MIT)
- [ ] **P0** `/SECURITY.md`
- [x] **P1** `/docs/architecture.md` ✅ —— 整体架构说明
- [x] **P1** `/docs/design-tokens.md` ✅ —— 设计令牌(色板/间距/字号/阴影/圆角/动画)
- [x] **P1** `/docs/theming.md` ✅ —— 主题定制化(浅色/深色/品牌色/字体)
- [x] **P1** `/docs/i18n.md` ✅ —— 国际化指南
- [x] **P1** `/docs/migration.md` ✅ —— 迁移指南(antd/MUI/Element Plus → Chaos UI)
- [x] **P1** `/docs/performance.md` ✅ —— 性能优化指南
- [x] **P1** `/docs/testing.md` ✅ —— 测试编写指南
- [ ] **P2** `/docs/faq.md`
...
- [x] **P1** React Server Components 兼容性 ✅(已开 `rsc: true`,验证)
...
- [x] **P1** VS Code 工作区推荐 ✅ `.vscode/settings.json`
  - Tailwind IntelliSense
  - ESLint
  - Prettier
  - 文件关联
- [x] **P1** VS Code 扩展推荐 ✅ `.vscode/extensions.json`
...
- 维护人:Chaos UI Team
- 最后更新:2026-06-30(v1.0.0-beta.0 稳定化基线发布:lint 0 error / 237 tests / 覆盖率 22.6% / 154 组件缺口核实)
- 当前版本:1.0.0-beta.0
- 目标版本:1.0.0 GA
...
- [x] **P0** `master-detail-tabs` ✅ —— **页面内多明细 tab**(本任务核心需求)
...
- [x] **P1** `subform-tabs` ✅ —— 子表单 tab(区别于明细 grid tab,数据是嵌套表单)
...
- [x] **P0** `page-header` ✅ 增强 —— 添加 fixed 吸顶(参考 `div[id$="Edit-TOOLBAR"]` CSS)
...
- [x] **P1** `print-button` ✅ / `print-service` —— 打印集成
...
- [x] **P0** `master-edit-template` ✅ —— MasterEdit 模板
...
- [x] **P1** `master-list-template` ✅ —— MasterList 模板
...
- [x] **P1** `area-chart` ✅ —— 面积图(趋势+填充)
- [x] **P1** `bar-chart` ✅ —— 柱状图(分组/堆叠/横向)
- [x] **P1** `line-chart` ✅ —— 折线图(多线+标注)
- [x] **P1** `donut-chart` ✅ —— 环形图(占比)
- [x] **P1** `pie-chart` ✅ —— 饼图
- [x] **P1** `scatter-chart` ✅ —— 散点图
- [x] **P1** `radial-chart` ✅ —— 径向图(达成率)
- [x] **P1** `spark-chart` ✅ —— 迷你趋势图(内联到 stat-card)
- [x] **P1** `funnel-chart` ✅ —— 漏斗图(转化率)
- [ ] **P2** `gauge-chart` —— 仪表盘图
- [x] **P2** `treemap-chart` ✅ —— 树图(层级占比)
- [x] **P2** `sankey-chart` ✅ —— 桑基图(流量流转)
- [x] **P2** `heatmap-chart` ✅ —— 热力图(已有 `heatmap-calendar.tsx`,增强为通用)
- [x] **P2** `radar-chart` ✅ —— 雷达图(多维评估)
- [x] **P2** `waterfall-chart` ✅ —— 瀑布图(财务分析)
...
- [x] **P1** `chart-card` ✅ —— 图表+标题+描述+KPI 一体卡片
- [x] **P1** `stat-card-with-sparkline` ✅ —— 统计卡 + 迷你趋势线
- [x] **P1** `stat-card-with-delta` ✅ —— 统计卡 + 增减标签(+12.5%↑)
- [x] **P1** `bar-list-card` ✅ —— 排名列表卡
- [x] **P1** `donut-card` ✅ —— 占比分析卡
- [x] **P2** `dashboard-grid` ✅ —— 预设仪表盘网格布局(2×2 / 3×2 / 自定义)
- [x] **P2** `overview-page` ✅ —— 总览页面模板(N 个 chart-card + stat-card-row)
